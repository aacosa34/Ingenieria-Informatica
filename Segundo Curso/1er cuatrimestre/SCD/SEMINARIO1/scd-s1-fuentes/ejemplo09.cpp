...
{
   double suma = 0.0;
   //Bucle que recorre cada suma parcial en cada uno de los n trozos
   for(long i=(ih*m)/n; i<((ih+1)*m)/n; i++){
      const double xi = double((i+0.5)/m);
      suma += f(xi);
   }
   return suma/m;
}
...
double calcular_integral_concurrente( )
{
  ...
  // Creamos un vector de futuros para almacenar cada resultado de las hebras
  future<double> futuros[n];

  for(int i = 0; i<n; i++) // Bucle para lanzar las hebras
      futuros[i] = async( launch::async, funcion_hebra, i);
   
  for(int i = 0; i<n; i++) // Bucle para buscar el resultado
      suma_integral += futuros[i].get();
